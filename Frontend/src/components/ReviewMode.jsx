...
export default function ReviewMode({ devMode, onQueueChanged, dueCount }) {
  ...
  const handleSkipToDue = useCallback(async () => {
    setSubmitting(true);
    try {
      await devSkipToDue();
      await loadQueue();
  ...
  const devHelper = devMode && (
    <div className="max-w-2xl mx-auto px-4 pt-2">
      <div className="rounded-xl border border-indigo-200 bg-indigo-600 px-4 py-3 shadow-sm flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        <p className="text-sm font-semibold text-white">
          🧪 Dev mode: 1 min for Needs work, 3 min for Got it right
        </p>
        <button
          className="px-3 py-1.5 rounded-lg bg-white/90 text-indigo-700 text-sm font-semibold hover:bg-white transition-colors disabled:opacity-60"
  ...
  if (!currentWord) {
    return (
      <div className="text-center py-16 flex flex-col items-center gap-6 max-w-2xl mx-auto">
        {devHelper}
        <div className="w-20 h-20 bg-gradient-to-br from-green-100 to-emerald-100 text-green-600 rounded-full flex items-center justify-center text-6xl shadow-lg">
          ✓
        </div>
        ...
        {lastOutcome && (
          <p className="text-sm font-mono text-gray-500 bg-gray-50 px-4 py-2 rounded-lg">
            Last review scheduled for {lastOutcome.intervalApplied}{" "}
            {lastOutcome.unit} from now.
          </p>
        )}
        {devMode && (
          <button
            className="mt-4 px-5 py-2.5 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 transition-all font-semibold"
            onClick={handleSkipToDue}
            disabled={submitting}
          >
            Make due now
    ...
    <div className="max-w-2xl mx-auto px-4 py-8 flex flex-col gap-8">
      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="text-base text-gray-600 mt-2">
            Reveal the definition and test your recall.
          </p>
        </div>
        {devMode && (
          <button
            className="text-xs uppercase px-4 py-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 transition-all font-bold whitespace-nowrap"
            onClick={handleSkipToDue}
            disabled={submitting}
          >
            Refresh queue
        ...
        <div className="relative w-full h-72 sm:h-96 transition-all duration-500 perspective">
          {!revealed ? (
            <div className="absolute inset-0 bg-gradient-to-br from-indigo-500 via-purple-500 to-pink-500 rounded-2xl p-10 flex flex-col items-center justify-center text-center shadow-2xl border-4 border-white/20 transform hover:scale-105 transition-transform">
              <span className="text-xs uppercase tracking-wider text-white/80 font-bold mb-6">
                What does this mean?
              </span>
              <h2 className="text-6xl md:text-7xl font-black text-white mb-4 drop-shadow-lg">
                {currentWord.word}
              </h2>
              {currentWord.partOfSpeech && (
                <span className="text-sm bg-white/20 text-white px-4 py-2 rounded-full mb-8 font-semibold backdrop-blur">
                  /{currentWord.partOfSpeech}/
                </span>
              )}
              <button
                className="px-8 py-4 bg-white text-indigo-600 rounded-xl font-bold hover:shadow-2xl transition-all transform hover:scale-110 active:scale-95 text-lg flex items-center gap-3 justify-center"
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        >
          ...
          <div className="text-xs mt-1 opacity-70">
            again in {devMode ? "3 min" : "3 days"}
          </div>
        </button>
      </div>

      {!revealed && (
        <p className="text-center text-base text-gray-600 bg-blue-50 px-6 py-4 rounded-lg border-l-4 border-blue-400 font-medium">
          💡 Reveal the definition before answering to strengthen your recall.
        </p>
      )}
    </div>
  );

...
export function isDue(word: VocabWord, atMs?: number): boolean {
  const checkTime = atMs ?? now();
  return new Date(word.nextReviewDate).getTime() <= checkTime;
}

export function addWord(input: { word: string; definition: string; example: string }): VocabWord {
...
export function reviewWord(id: string, result: 'correct' | 'needs_work'): VocabWord {
  const words = getWords();
...
export function advanceTimeDays(days: number): void {
...
// React hooks
export function useWords(): VocabWord[] {
...
export function useDueWords(): VocabWord[] {
